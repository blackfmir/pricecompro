...
def dashboard(request: Request, db: DBSession):
    ...
    return templates.TemplateResponse("dashboard.html", {"request": request, "total_sup": total_sup, "total_pl": total_pl, "total_sp": total_sp})
...
def ui_suppliers_create(request: Request, db: DBSession, name: str = Form(...), code: str = Form(...), active: bool = Form(True)):
...
def ui_price_lists(request: Request, db: DBSession):
    ...
    return templates.TemplateResponse("price_lists_list.html", {"request": request, "items": items, "suppliers": suppliers})

@router.post("/ui/price-lists")
def ui_price_lists_create(request: Request, db: DBSession, supplier_id: int = Form(...), name: str = Form(...), format: str = Form("xml")):
    payload = PriceListCreate(
        ...
        mapping={
            "product_fields": {
                "supplier_sku": {"by": "xpath", "value": "./attributes/attribute[@eid='TRADE_INDEX']/value/text()"},
...
async def ui_import_upload(request: Request, db: DBSession, pl_id: int, file: FileUpload):
    ...
    if fmt in {"xlsx", "xls"}:
        ...
        items, errors = import_xml_bytes(
            ...
            supplier_id=pl.supplier_id,
            price_list_id=pl.id,
            mapping=cast(dict[Any, Any], mapping_dict),  # якщо сигнатура ширша
            source_config=source_cfg,
        )
    else:
        items, errors = [], [f"Format {fmt} not supported in UI; use API."]
...
def ui_supplier_products(request: Request, db: DBSession, supplier_id: int | None = None, q: str | None = None, page: int = 1, per_page: int = 50):
    offset = (page - 1) * per_page
    data = sp_crud.list_(db, supplier_id=supplier_id, q=q, limit=per_page, offset=offset)
    return templates.TemplateResponse("supplier_products.html", {"request": request, "items": data["items"], "total": data["total"], "page": page, "per_page": per_page, "supplier_id": supplier_id, "q": q})
